...
def _derive_prior_cutoff(project_root: Path, previous_run_ids: List[str],
    ...
    共享 lineage helper（P7-D1-R3）：本函数仅转发到
    research_os.review.prior_run_lineage.derive_prior_cutoff（无业务语义变化）。
    """
    from research_os.review.prior_run_lineage import derive_prior_cutoff as _shared
    return _shared(project_root, previous_run_ids, as_of)
