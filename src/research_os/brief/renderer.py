"""Brief（晨报/晚报）Markdown 渲染（Phase 2 任务 18 节模板 + Phase 6B 同构复用）。

Morning/Evening 共用同一渲染模板：唯一差异为 scenario 标识与报告标题前缀
（由 BriefWindowPolicy 提供）。结构化数据中必须包含完整字段（分类/监测方向/
信息性质/时间/主体/摘要/重要性/影响路径/确定性/待验证/证据）；正文可适度压缩。
"""
...
from research_os.brief.pipeline import PipelineArtifacts
...
def _frontmatter(artifacts: PipelineArtifacts, report_date: date,
                 ...
                 delayed: bool, delay_seconds: int,
                 scenario: str, title_prefix: str) -> List[str]:
    status = "partial" if artifacts.missing_data else "ok"
    lines = [
        "---",
        f"report_id: {artifacts.task_id}",
        f"scenario: {scenario}",
        f"title: {title_prefix} {report_date.isoformat()}",
        ...
        "source_coverage: {}",
        # 诚实记录模型路由：Brief 流水线未接入 LLM，语义环节为确定性规则回退
        "model_route:",
...
def render_brief(
    ...
    *,
    ...
    title_prefix: str = "A股每日晨报",
) -> str:
    """渲染完整 Brief Markdown（含 Front Matter）。

    morning_brief 与 evening_brief 共用本渲染器；scenario / title_prefix
    由窗口策略提供，其余章节结构、过滤与数量控制完全一致。
    """
    finished = now_iso()
    out: List[str] = _frontmatter(
        ...
        scenario, title_prefix)
    out += ["", f"# {title_prefix} {report_date.isoformat()}", ""]
