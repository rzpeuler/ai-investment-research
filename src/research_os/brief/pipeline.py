"""Brief（晨报/晚报）共享信息处理链（Phase 2 任务 4 节 + Phase 6B 同构复用）。

来源采集 -> RawItem -> 时间窗口过滤 -> 精确去重 -> 语义聚类 -> 分类 ->
硬性否决 -> 评分 -> 事件簇合并 -> Claim 生成 -> 选择 -> Markdown -> 校验。

morning_brief 与 evening_brief 共用本流水线（DECISIONS #43）：唯一业务差异为
信息采集时间窗口，由注入的 BriefWindowPolicy 决定；其余环节完全一致，不复制
第二套晨报 Pipeline。
...
class BriefPipeline:
    """晨报/晚报共享流水线（确定性核心；LLM 环节以规则回退实现，接口留扩展）。

    window_policy 注入信息窗口与报告身份：morning_brief 传 morning_policy()，
    evening_brief 传 evening_policy()；默认 morning 策略（Phase 2 行为不变）。
    """

    def __init__(self, config: Optional[PipelineConfig] = None,
                 window_policy: Optional[BriefWindowPolicy] = None):
        self.config = config or PipelineConfig()
        self.window_policy = window_policy or morning_policy()
    ...
    def run(
        ...
        *,
        ...
        window_start, window_end = self.window_policy.window(report_date)
        started = started_at or now_iso()
        artifacts = PipelineArtifacts(task_id=task_id or new_uuid())

        # 0. 时间窗口过滤（窗口外且无新更新 -> 排除，写入 veto 理由）。
        #    语义 [start, end)：start 含、end 不含；延迟补跑不漂移；
        #    用户显式 as_of 早于窗口结束时以其为数据截止。
        window_start_dt = parse_iso(window_start)
        window_end_dt = parse_iso(window_end)
        as_of_dt = parse_iso(as_of or self.window_policy.as_of(report_date))
        cutoff = min(window_end_dt, as_of_dt)
        in_window = []
        for item in raw_items:
            try:
                published = parse_iso(item.published_at)
                if (published < window_start_dt or published >= window_end_dt
                        or published > cutoff):
        ...
        scheduled = self.window_policy.scheduled_for(report_date)
        delayed, delay_seconds = delay_info(started, scheduled)
        artifacts.markdown = render_brief(
            artifacts=artifacts, report_date=report_date,
            window_start=window_start, window_end=window_end,
            as_of=as_of or self.window_policy.as_of(report_date),
            scheduled_for=scheduled, started_at=started,
            delayed=delayed, delay_seconds=delay_seconds,
            scenario=self.window_policy.scenario_id,
            title_prefix=self.window_policy.title_prefix,
        )
        from research_os.brief.validation import validate_brief_evidence

        evidence_validation = validate_brief_evidence(artifacts)
        if evidence_validation.errors:
            raise ValueError("Brief Evidence 校验失败: " + "; ".join(evidence_validation.errors))
        artifacts.report_path = self.window_policy.report_path_for(
            report_date, str(Path.cwd() / "reports"))
...
# Phase 2 兼容别名：MorningBriefPipeline 即共享 BriefPipeline（默认 morning 策略）。
MorningBriefPipeline = BriefPipeline
