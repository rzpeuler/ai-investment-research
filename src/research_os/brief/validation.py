"""Brief（晨报/晚报）Claim/Evidence 机械校验（Phase 2 + Phase 6B 同构复用）。

morning_brief 与 evening_brief 共用同一校验规则；校验不依赖场景身份。
"""
...
@dataclass
class BriefEvidenceValidation:
    status: str
    errors: List[str] = field(default_factory=list)
    warnings: List[str] = field(default_factory=list)


def validate_brief_evidence(artifacts: Any) -> BriefEvidenceValidation:
    errors: List[str] = []
    warnings: List[str] = []
    ...
    return BriefEvidenceValidation("fail" if errors else "pass", errors, warnings)
