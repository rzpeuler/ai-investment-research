...
class ThemeDiscoveryPipeline:
    ...
    def run(self, request: Dict[str, Any]) -> ThemeDiscoveryResult:
        ...
        for theme in themes:
            self._populate_evidence_analysis(theme)
        ...
        has_evidence = any(t.supporting_evidence_ids for t in themes)
        result.status = "success" if (themes and has_evidence) else ("partial_success" if themes else "insufficient_evidence")
        result.model_route = {"mode": "deterministic_code", "llm_called": False}
        result.exit_code = 0
    ...
    def _build_hypothesis(
        ...
        return ThemeHypothesis(
            ...
            generated_by="deterministic_code", model_route="deterministic_code",
        )
    ...
    def _detect_lifecycle(self, theme: ThemeHypothesis) -> str:
        ...
        if theme.invalidating_conditions:
            return "invalidated"
        if theme.counter_evidence_ids or theme.counter_evidence:
            return "weakening"
    ...
    def _populate_evidence_analysis(self, theme: ThemeHypothesis) -> None:
        """填充 evidence 分析字段：
        supporting_evidence_ids, counter_evidence_ids, supporting_factors,
        counter_evidence, industry_mapping, related_entity_ids,
        invalidating_conditions, open_questions, uncertainty。
        """
        # ── supporting_evidence_ids ──
        ev_set: Set[str] = set()
        for t in theme.triggers:
            ev_set.update(t.evidence_ids)
        theme.supporting_evidence_ids = sorted(ev_set)
        ...
        if ev_set:
            factors.append(f"关联 {len(ev_set)} 条知识图谱证据")
        theme.supporting_factors = factors or theme.supporting_factors

        # ── counter_evidence ──
        counter: List[str] = []
        if len(theme.triggers) <= 1:
            counter.append("触发信号数量不足（仅 1 个），可能是噪声")
        if not ev_set:
            counter.append("无直接关联证据")
        weak = [t for t in theme.triggers if t.strength < 0.4]
        if weak:
            counter.append(f"{len(weak)} 个 trigger strength < 0.4，信号偏弱")
        theme.counter_evidence = counter or theme.counter_evidence
    ...
    def _compute_metrics(self, theme: ThemeHypothesis) -> ResearchSortMetrics:
        ...
        # controversy_level
        n_counter = len(theme.counter_evidence)
