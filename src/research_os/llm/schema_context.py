...
def build_harness_prompt(request, output_schema: dict[str, Any], *,
    ...
    """Harness prompt (R4-R2 iteration).

    Empirical benchmark finding (R4-R1): the completion checklist and
    self-validation instructions systematically REDUCED schema-valid rate
    (combined prompt: 0.2/0.2 across two runs) versus the R3 structure (full
    schema + required constraints + example: 0.5). This version keeps the
    measured-best R3 structure: JSON-only instruction + full schema +
    required-field constraints + a complete valid example + task/evidence
    context. The schema, validator and normalizer are never changed; the
