...
| Phase 5 | IN_PROGRESS | **PR5B MERGED**（squash merge → master `cfdeeba7`）。M0-M8 PASS（M8 SHA `eac18e2`，CI `31269460005`，2009 passed / 5 skipped / 0 xfail / 55/55 schemas / DB v6）。**PR5C** `phase5/pipeline-integration`（Draft PR #6）。**M9 AUTHORIZED / IN_PROGRESS**。M10 NOT_AUTHORIZED。
...
M0-M8 全部通过独立验收。
M9 Authorized / IN_PROGRESS（PR5C `phase5/pipeline-integration`，Draft PR #6）。
...
PR5B 已 squash merge（master `cfdeeba7`）。M0-M8 PASS。M9 AUTHORIZED / IN_PROGRESS（PR5C）。M10 NOT_AUTHORIZED。
