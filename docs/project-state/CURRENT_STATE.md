...
| Phase 5 | IN_PROGRESS | **PR5B MERGED**（squash merge → master `cfdeeba7`）。M0-M9 PASS（M9 SHA `d097ca8`，CI `31275096225`，2068 passed / 5 skipped / 0 xfail / 55/55 schemas / DB v6）。**PR5C** `phase5/pipeline-integration`（Draft PR #6）。**M10 AUTHORIZED / IN_PROGRESS**。
...
PR5B 已 squash merge（master `cfdeeba7`）。M0-M9 PASS。M10 AUTHORIZED / IN_PROGRESS（PR5C）。Phase5 IN_PROGRESS。
